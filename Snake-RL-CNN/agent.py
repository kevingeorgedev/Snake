...
class Agent:
    ...
    def __init__(self):
        ...
        #self.model   = Linear_QNet(11, 3, 3).to(torch.device('cuda:0'))
        self.model   = Linear_QNet().to(torch.device('cuda:0'))
...
def train():
    ...
    while True:
        ...
        if done:
            ...
            print(f"Game {agent.n_games} Score {score} Memory {len(agent.memory)} Record: {record}")
